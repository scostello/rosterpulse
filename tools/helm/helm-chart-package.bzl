...
load("//tools/helpers:helpers.bzl", "write_sh", "get_make_value_or_default")
...
def _helm_chart_impl(ctx):
    ...
    yq = ctx.toolchains["@rosterpulse//tools/toolchains/yq:toolchain_type"].yqinfo.tool.files.to_list()[0]
...
helm_chart = rule(
    ...
    toolchains = [
        "@rosterpulse//tools/toolchains/yq:toolchain_type",
